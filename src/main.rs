...
fn main() {
    ...
    loop {
        ...
        if let Some(cpu_temperature) = performance::cpu::get_cpu_temperature() {
            println!("CPU Temperature: {} ºC", cpu_temperature)
        }
        ...
        for partition in performance::disk::get_disk_partitions_usage() {
        ...
        if let Some(net_usage) = performance::net::calc_net_interval(first_net_usage, second_net_usage) {
